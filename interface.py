...
def gera_grafico(serie_dados, indice, restricao, tam_cel, tam_rio, classe):
    ...
    if indice == 0:
        ...
        ax.legend(['DBO', 'Limite da Classe ' + str(classe.get())])
    elif indice == 1:
        fig.suptitle("Oxigênio Dissolvido")
        ax.legend(['OD', 'Limite da Classe ' + str(classe.get())])
    elif indice == 2:
        fig.suptitle("Nitrogênio Orgânico")
        ax.legend(['NORG'])
    elif indice == 3:
        fig.suptitle("Nitrogênio Amoniacal")
        ax.legend(['NH3', 'Limite da Classe ' + str(classe.get())])
    elif indice == 4:
        fig.suptitle("Nitrito")
        ax.legend(['Nitrito', 'Limite da Classe ' + str(classe.get())])
    elif indice == 5:
        fig.suptitle("Nitrato")
        ax.legend(['Nitrato', 'Limite da Classe ' + str(classe.get())])
    elif indice == 6:
...
def executa(vetor_caminhos, diretorio_saidas, modo_execucao, label_caixa_texto, classe, modo_otimizacao):
    ...
    if modo_execucao.get() == "110":
        ...
        Main_brkga.checa_arquivo(diretorio_saidas,"Perfil de QA-Poluição Difusa.csv")

        # CRIACAO DO CENARIO BASE DIFUSO
        qual_ufmg = QUAL_UFMG()
        [rio, matriz_contribuicoes, matriz_reducoes, scs, tam_rio, tam_cel, ph, matriz_tipo_contribuicoes] = qual_ufmg.constroi_param_ini(vetor_caminhos[4],
                                                                                                               vetor_caminhos[5], True,
                                                                                                               vetor_caminhos[2],
                                                                                                               vetor_caminhos[0],
                                                                                                               vetor_caminhos[1],
                                                                                                               vetor_caminhos[3], classe)
        # qual_ufmg.constroi_param_ini(pontual, constantes, simula_difusa, cn, hidro, cme, usos)

        cenario_base_difuso = qual_ufmg.executa(rio, matriz_contribuicoes, matriz_reducoes, True, [], scs, None, matriz_tipo_contribuicoes)
        # qual_ufmg.executa(rio, matriz_contribuicoes, matriz_reducoes, simula_difusa, matriz_reducao_difusa, scs)

        for i in range(len(cenario_base_difuso)):
            cenario_base_difuso[i].Escreve_saida_PD(diretorio_saidas + "\Perfil de QA-Poluição Difusa - Tributario " + str(i+1) + ".csv")
        ...
        # Guardando perfil da vazao
        perfilQP = None
        perfilQ = cenario_base_difuso[i].PerfilQ
        ...
        caminho_saida_qualufmg = diretorio_saidas + "\Perfil de QA-Poluição Pontual.csv"
        caminho_saida_pd = diretorio_saidas + "\Perfil de QA-Poluição Difusa.csv"
        Main_brkga.checa_arquivo(diretorio_saidas,"Perfil de QA-Poluição Pontual.csv")
        Main_brkga.checa_arquivo(diretorio_saidas,"Perfil de QA-Poluição Difusa.csv")
        ...
        cenario_base_difuso.Escreve_saida_PD(caminho_saida_pd)

        # CRIACAO DO CENARIO BASE PONTUAL
        qual_ufmg = QUAL_UFMG()
        [rio, matriz_contribuicoes, matriz_reducoes, scs, tam_rio, tam_cel, ph, matriz_tipo_contribuicoes] = qual_ufmg.constroi_param_ini(vetor_caminhos[4],
                                                                                                           vetor_caminhos[5], False, "",
                                                                                                           "", "", "", classe)
        del scs  # Parametro nulo

        cenario_base_pontual = qual_ufmg.executa(rio, matriz_contribuicoes, matriz_reducoes, False, [], [], None, matriz_tipo_contribuicoes)
        # qual_ufmg.executa(rio, matriz_contribuicoes, matriz_reducoes, simula_difusa, matriz_reducao_difusa, scs)

        cenario_base_pontual.Escreve_saida_QualUFMG(caminho_saida_qualufmg, tam_rio, tam_cel)

        # Guardando perfil da vazao
        perfilQP = cenario_base_pontual.PerfilQ
        perfilQ = cenario_base_difuso.PerfilQ
        ...
        scs = SCS(vetor_caminhos[2], vetor_caminhos[0], vetor_caminhos[1], vetor_caminhos[3])
        # scs = SCS(cn, hidro, CME, usos)

        leitor = Leitura()
        tam_cel = float(leitor.ler_tamanho_celula(vetor_caminhos[0]))
        ...
        caminho_saida_pd = diretorio_saidas + "\Poluição Difusa e CME Ponderada-Cenário Base.csv"
        Main_brkga.checa_arquivo(diretorio_saidas,"Poluição Difusa e CME Ponderada-Cenário Base.csv")
        scs.Escreve_saida_PD(caminho_saida_pd)
        ...
        [cenario_base_pontual, cenario_otimizado_pontual, cenario_base_difuso, cenario_otimizado_difuso,
         populacoes, historico_foP, historico_fo, historico_tempo_iteracoesP, historico_tempo_iteracoes,
         historico_filhos_invalidosP, historico_filhos_invalidos, iteracao_versao_rapidaP, iteracao_versao_rapida,
         tam_rio, tam_cel, ph, perfilQP, perfilQ] = brkga.executa(vetor_caminhos, diretorio_saidas, modo_execucao.get(), classe, matriz_brkga[0][6], matriz_brkga[1][6], modo_otimizacao.get())
