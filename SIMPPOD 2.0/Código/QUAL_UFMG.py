...
class QUAL_UFMG(object):
    ...
    def constroi_param_ini(arquivo, simula_difusa):
        ...
        # const = [tam_rio, tam_cel, classe, T, Altitude, Ph, DBOr, ODr, NORGr, NAMONr, NNITRIr, NNITRAr, PORGr, PINORGr, COLr,
        #          Qr, Ro2a, Ks, K1, Kb, Koa, Kan, Koi, Kspo, Samon, Kso, Knn, Ro2n, Knitr, Sinorg, a_vel, b_vel, a_prof,
        #          b_prof, DBOinc, ODinc, NORGinc, NAMONinc, NNITRIinc, NNITRAinc, PORGinc, PINORGinc, COLinc, Qinc, Sd, Lrd, KM_JUSANTE, 
        #          ID, ID_Jusante]

        rio = Rio(const[0], const[1], const[2], const[3], const[4], const[5], const[6], const[7], const[8], const[9], const[10], const[11],
                  const[12], const[13], const[14], const[15], const[16], const[17], const[18], const[19], const[20], const[21], const[22],
                  const[23], const[24], const[25], const[26], const[27], const[28], const[29], const[30], const[31], const[32], const[33],
                  const[34], const[35], const[36], const[37], const[38], const[39], const[40], const[41], const[42], const[43], const[44],
                  const[45], const[46], const[47], const[48])
        ...
        for i in range(len(matriz_contribuicoes)): matriz_contribuicoes[i][10] *= 1000 / const[1]
        for i in range(len(matriz_reducoes[1])): matriz_reducoes[1][i] *= 1000 / const[1]
    ...
    def aplica_reducoes(matriz_contribuicoes, matriz_reducao_pontual, numFuncao):
        ...
        while i < len(matriz_contribuicoes): # and j < len(matriz_reducao_pontual[1]):
            ...
            if j < len(matriz_reducao_pontual[1]) and matriz_contribuicoes[i][10] == matriz_reducao_pontual[1][j]:
                novo_vetor = matriz_contribuicoes[i].copy()
                if numFuncao == 1:
                ...
                matriz_contribuicoes_otimizada.append(novo_vetor) 
                j += 1
                i += 1
            else:
                matriz_contribuicoes_otimizada.append(matriz_contribuicoes[i]) 
                i += 1

        return matriz_contribuicoes_otimizada

    def executa(self, caminho_curso, simulacao_curso, Rio, matriz_contribuicoes, matriz_reducao_pontual, simula_difusa, matriz_reducao_difusa, scs, numFuncaoP, matriz_tipo_contribuicoes, celula_entrada_tributarios):
        euler = Euler()
        ...
        if simula_difusa:
            ...
            cenario = euler.Calcula(caminho_curso, simulacao_curso, Rio, matriz_final, matriz_tipo_contribuicoes, simula_difusa, celula_entrada_tributarios)
            ...
            if matriz_reducao_pontual[0]:
                matriz_contribuicoes_otimizada_tributario = self.aplica_reducoes(matriz_contribuicoes, matriz_reducao_pontual, numFuncaoP)
                cenario = euler.Calcula(caminho_curso, simulacao_curso, Rio, matriz_contribuicoes_otimizada_tributario, matriz_tipo_contribuicoes, simula_difusa, celula_entrada_tributarios)
            else:
                cenario = euler.Calcula(caminho_curso, simulacao_curso, Rio, matriz_contribuicoes, matriz_tipo_contribuicoes, simula_difusa, celula_entrada_tributarios)

        for i in range(len(cenario.Y_Coliformes)): cenario.Y_Coliformes[i] = cenario.Y_Coliformes[i]/10  # passa de L para mL
