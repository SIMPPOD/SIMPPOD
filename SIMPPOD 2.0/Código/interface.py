...
from tkinter import Canvas, Frame, Label, Entry
...
def calcular(entrada):
    valor_digitado = entrada.get()  # Obter o valor digitado pelo usuário
    # Fazer alguma operação com base no valor digitado
    resultado = int(valor_digitado) * 2
    # Exibir o resultado em uma caixa de diálogo
    showinfo("Resultado", f"O resultado é: {resultado}")
...
def Janela_Simula_Escolhe_Otimizacao(janela_inicial, Tela, simula_rio, simula_tributarios):
    ...
    canvas = Canvas(frame, height=205, width=946, bg='#94B7D5', highlightbackground="black")
    ...
    checa_nsgaii = Radiobutton(canvas, text="Otimização com NSGA-II", bg='#94B7D5', font=('Arial', '14'), variable=modo_otimizacao, value=4)
    checa_nsgaii.place(x=2, y=165)
...
def Janela_Simula_Otimizacao(janela_inicial, Tela, modo_otimizacao, simula_rio, simula_tributarios):
    ...
    elif modo_otimizacao.get() == 3:
        titulo["text"] = "Otimização com BRKGA:"
    else:
        titulo["text"] = "Otimização com NSGA-II"
...
def Janela_Simula_Otimizacao_Modo(janela_inicial, Tela, modo_simulacao, modo_otimizacao, simula_rio, simula_tributarios):
    ...
    if modo_execucao.get() == "21":
        if modo_otimizacao.get() == 1:
            ...
            titulo["text"] = "Otimização de Fontes Pontuais com NSGA-II"
        ...
        if modo_otimizacao.get() == 1:
            ...
            titulo["text"] = "Otimização de Fontes Pontuais e de Fontes Difusas com NSGA-II"
...
def Janela_saidas(janela_inicial, Tela, vetor_caminhos, diretorio_saidas, modo_execucao, modo_otimizacao, simula_rio, simula_tributarios):
    ...
    if diretorio_saidas == "" or (simula_rio.get() and vetor_caminhos[0] == "") or (simula_tributarios.get() and vetor_caminhos[1] == ""):
        ...
        tam_rio, tam_cel, ph, perfilQP, perfilQ, melhor_solucao_pontual_rapida, vetor_vazao, vetor_dbo, vetor_od] = executa(vetor_caminhos, diretorio_saidas, modo_execucao, label_caixa_textos, modo_otimizacao, simula_rio, simula_tributarios)
        ...
        if simula_rio.get() or (not simula_rio.get() and len(vetor_caminhos[1]) == 1):
            if modo_execucao.get() != 2:
                ...
                tam_rio = len(vetor_vazao) * tam_cel
            ...
            if modo_execucao.get() == "110":
                ...
                botao_vazaoP["command"] = partial(gera_grafico_vazao, vetor_vazao, tam_cel, tam_rio)

                botao_cbp_DBO["command"] = partial(gera_grafico, vetor_dbo, 0, restricao_DBO, tam_cel, tam_rio, classe)  # DBO
                botao_cbp_OD["command"] = partial(gera_grafico, vetor_od, 1, restricao_OD, tam_cel, tam_rio, classe)  # OD
                ...
                botao_cbp_COL["command"] = partial(gera_grafico, cenario_base_pontual.Y_Coliformes, 8, restricao_COL, tam_cel, tam_rio, classe)
                ...
                botao_cop_COL["command"] = partial(gera_grafico, melhor_solucao_pontual.cenario.Y_Coliformes, 8, restricao_COL, tam_cel, tam_rio, classe)
