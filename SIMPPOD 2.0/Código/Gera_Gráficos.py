...
def gera_grafico(serie_dados, indice, restricao, tam_cel, tam_rio, classe):
    ...
    num_cel = len(serie_dados)
    tam_rio = len(serie_dados) * tam_cel
    x = []
...
def gera_grafico_fo(serie_dados, iteracao_versao_rapida):
    ...
    x = []
    for i in range(len(serie_dados)):
        x.append(i+1)

    if type(serie_dados[0]) is list: serie_dados = [serie_dados[i][0] for i in range(len(serie_dados))]
    ax.plot(x, serie_dados, 'b')

    if iteracao_versao_rapida is not None:
        ax.axvline(x=iteracao_versao_rapida+1, ymin=0, ymax=1, color='g')
        ax.legend(['FO', 'Última Iteração com Critério de Parada = ' + str(iteracao_versao_rapida+1)], loc='upper right')
    else:
...
def gera_grafico_tempo_iteracoes(serie_dados, iteracao_versao_rapida):
    ...
    fig = Figure(figsize=(6, 4), dpi=100)
    ax = fig.add_subplot(111, xlabel="Iteração", ylabel="Tempo (s)")
    fig.suptitle('Comportamento da Duração da Iteração')
...
def gera_grafico_filhos_invalidos(serie_dados, iteracao_versao_rapida):
    ...
    fig = Figure(figsize=(6, 4), dpi=100)
    ax = fig.add_subplot(111, xlabel="Iteração", ylabel="Filhos Inválidos (u)")
    fig.suptitle('Comportamento da Geração de Filhos Inválidos')
    ...
    maior = max(serie_dados)
    media = sum(serie_dados)/len(serie_dados)
    soma = sum(serie_dados)

    fig.text(0.01, 1, 'Mínimo = ' + str(round(menor,4)) +
                        ' | Máximo = ' + str(round(maior,4)) +
                        ' | Média = ' + str(round(media,4)) + 
                        ' | Soma = ' + str(round(soma,4)),
            horizontalalignment='left', verticalalignment='bottom', transform = ax.transAxes)
